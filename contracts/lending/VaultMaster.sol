...
contract VaultMaster is IVaultMaster, ExponentialNoError, Ownable {
  ...
  function mint_vault() public returns (address){
    ...
    address vault_address = address(new Vault(_vaultsMinted, msg.sender, address(this), _usdiAddress));
  ...
  function register_oracle_master(address master_oracle_address) external onlyOwner {
    _oracleMasterAddress = master_oracle_address;
    _oracleMaster = OracleMaster(_oracleMasterAddress);
  }

  function getInterestFactor() override external view returns (uint256) {
    return _e18_interestFactor;
  }

  function register_erc20(address token_address, uint256 LTVe4, address oracle_address) external onlyOwner {
  ...
  function check_account(uint256 id) override external view returns (bool) {
    ...
    uint256 usdi_liability = vault.getBaseLiability() * _e18_interestFactor / 1e18;
  ...
  function borrow_usdi(uint256 id, uint256 amount) override external {
    address vault_address = _vaultId_vaultAddress[id];
    require(vault_address != address(0x0), "vault does not exist");
    IVault vault = IVault(vault_address);
    require(msg.sender == vault.getMinter(), "only vault creator may borrow from their vault");
  ...
  function repay_all_usdi(uint256 id) override external{
    ...
    Exp memory interest_factor = Exp({mantissa: _e18_interestFactor});
    uint256 usdi_liability = ExponentialNoError.mul_ScalarTruncate(interest_factor,vault.getBaseLiability());
    vault.decrease_liability(vault.getBaseLiability());
    _usdi.vault_master_burn(msg.sender,usdi_liability);
  }

  function liquidate_account(uint256 id, address asset_address, uint256 max_usdi) override external returns (uint256) {
    address vault_address = _vaultId_vaultAddress[id];
    require(vault_address != address(0x0), "vault does not exist");
    ...
    uint256 asset_price = uint256(_oracleMaster.get_live_price(asset_address));
    ...
    uint256 tokens_to_liquidate = ExponentialNoError.div_(usdi_to_repurchase, price);
    ...
    if(tokens_to_liquidate > vault_balance){
      ...
      usdi_to_repurchase = ExponentialNoError.mul_ScalarTruncate(price,tokens_to_liquidate);
    }
    // upon liquidating all this, we first must find out how much usdi we have to work with.
    uint256 total_proceeds = ExponentialNoError.mul_ScalarTruncate(price,tokens_to_liquidate);
    ...
    IERC20(asset_address).transferFrom(address(this),msg.sender,tokens_to_liquidate);
  ...
  function get_vault_collateral_value(IVault vault) private view returns (uint256){
    ...
    for(uint256 i = 0; i < _tokensRegistered; i++){
      ...
      if(raw_price != 0){
        ...
        uint256 token_value = ExponentialNoError.mul_ScalarTruncate(Exp({mantissa: raw_price}),balance) * //
          _tokenId_tokenLTVe4[i] / 1e4; //
        total_liquidity_value = total_liquidity_value + token_value;
      }
    }
  ...
  function pay_interest() private {
    ...
    uint256 valueAfter = _totalBaseLiability  * _e18_interestFactor;
