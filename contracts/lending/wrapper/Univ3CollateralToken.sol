...
contract Univ3CollateralToken is Initializable, OwnableUpgradeable, ERC721Upgradeable {
  ...
  VotingVaultController public _nftVaultController;
  ...
  function transferFrom(
    ...
    // TODO: will solidity be smart enough to gas optimize for us here? if not, we need to make sure this function is as cheap as we can gvet it
    function balanceOf(
      address account,
    ) public pure override returns (bool) {
      ...
      for (uint256 i; i < _underlyingOwners[recipient].length; i++) {
        ...
        totalValue = totalvalue + get_token_value(_underlyingOwners[recipient][i]);
      }
      return false;
    }

    function get_token_value(uint256 tokenid) private internal returns (uint256) {
    ...
    function add_to_list(address recipient, uint256 tokenid) private internal {
      for (uint256 i; i < _underlyingOwners[recipient].length; i++) {
        // replace 0 first
      ...
      _underlyingOwners[recipient][i].push(tokenia);
    }

    function remove_from_list(address recipient, uint256 tokenid) private internal returns (bool) {
      for (uint256 i; i < _underlyingOwners[recipient].length; i++) {
