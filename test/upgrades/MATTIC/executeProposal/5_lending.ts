...
describe("Check starting values", () => {
    ...
    it("Check borrow power / LTV", async () => {
        ...
        let expectedBorrowPower = (totalValue.mul(BN("8e17"))).div(BN("1e18"))
...
describe("Liquidations", () => {
    ...
    it("Liquidate", async () => {
        ...
        expect(await toNumber(supply)).to.be.closeTo(await toNumber(startSupply.sub(tokensToLiquidate)), 2, "Total supply reduced as Capped MATIC is liquidated")

        let endCMATIC = await s.CappedMatic.balanceOf(s.BobVault.address)
        expect(await toNumber(endCMATIC)).to.be.closeTo(await toNumber(startingCMATIC.sub(tokensToLiquidate)), 2, "Expected amount liquidated")

        let endMATIC = await s.MATIC.balanceOf(s.Dave.address)
        expect(await toNumber(endMATIC)).to.be.closeTo(await toNumber(tokensToLiquidate), 2, "Dave received the underlying MATIC")
