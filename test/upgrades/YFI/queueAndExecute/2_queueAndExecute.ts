...
describe("Deploy Cap Tokens and Oracles", () => {
  ...
  it("Deploy capped YFI", async () => {
    s.CappedYFI = await DeployContractWithProxy(
    ...
    await s.CappedYFI.deployed()

    await s.CappedYFI.connect(s.Frank).setCap(s.YFI_CAP)

    await s.CappedYFI.connect(s.Frank).transferOwnership(s.owner._address)
  })

  it("Deploy YFI oracle system", async () => {

    //chainYFI oracle
    const chainYFIDataFeed = "0xa027702dbb89fbd58938e4324ac03b58d812b0e1"

    const chainYFIRelay = await new ChainlinkOracleRelay__factory(s.Frank).deploy(
      chainYFIDataFeed,
      BN("1e10"),
      BN("1")
    )
    await chainYFIRelay.deployed()
    showBody("ChainYFI data feed price: ", await toNumber(await chainYFIRelay.currentValue()))

    //uni v3 oracle
    const uniPool = "0x2E8dAf55F212BE91D3fA882ccEAb193A08fddeB2"//10k YFI/wETH pool
    ...
    anchorViewYFI = await new AnchoredViewRelay__factory(s.Frank).deploy(
      uniRelay.address,
      chainYFIRelay.address,
      BN("5"),
      BN("100")
    )
    await anchorViewYFI.deployed()
    showBodyCyan("ANCHOR VIEW PRICE: ", await toNumber(await anchorViewYFI.currentValue()))
...
describe("Setup, Queue, and Execute proposal", () => {
  ...
  it("Makes the new proposal", async () => {
    ...
    const proposal = new ProposalContext("YFI")

    const addOracleYFI = await new OracleMaster__factory(prop).
      attach(s.Oracle.address).
      populateTransaction.setRelay(
        s.CappedYFI.address,
        anchorViewYFI.address
      )

    const listYFI = await new VaultController__factory(prop).
    ...
    const registerYFI_VVC = await new VotingVaultController__factory(prop).
