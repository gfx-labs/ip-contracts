...
describe("hardhat settings", () => {
    it("Set hardhat network to a block after deployment", async () => {
        expect(await reset(15711906)).to.not.throw;//14940917
    });
...
describe("Token Setup", () => {
    before(async () => {
        const LPS = require('../../../rewardtree/mergedAndFormatWeek15.json')

        s.mergedList = LPS

        const LPS2 = require('../../../rewardtree/mergedAndFormatWeek155.json')
        s.mergedList2 = LPS2
    })
    ...
    it("Connect to mainnet deploy for MerkleRedeem contract, and initialize root", async () => {
        ...
        leafNodes = s.mergedList2.map((obj) =>
            solidityKeccak256(["address", "uint256"], [obj.minter, BN(obj.amount)])
        )
        s.MERKLE_TREE2 = new MerkleTree(leafNodes, keccak256, { sortPairs: true })
        s.ROOT2 = s.MERKLE_TREE2.getHexRoot()
