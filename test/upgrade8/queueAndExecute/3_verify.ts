...
describe("Testing CappedToken functions", () => {
    ...
    it("Deposit underlying", async () => {
        ...
        caBalance = await s.CappedLDO.balanceOf(s.Bob.address)
        expect(caBalance).to.eq(0, "Bob holds 0 capped LDO after deposit")
    ...
    it("No vault", async () => {
        ...
        showBody("Got balance:", await toNumber(startBal))
        expect(startBal).to.eq(s.LDO_Amount, "Balance correct")
        showBody("Balance Correct")
    ...
    /**
       it("Try to withdraw more capped LDO than is possible given some cap tokens", async () => {
        ...
        await s.LDO.connect(s.Gus).approve(s.CappedLDO.address, BN("1e18"))
        await mineBlock()
        showBody("Approved")
