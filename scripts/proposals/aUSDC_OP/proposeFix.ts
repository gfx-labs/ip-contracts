...
import {
    ...
    GovernorCharlieDelegate__factory, ILayer1Messenger__factory, InterestProtocolTokenDelegate__factory, OracleMaster__factory, ProxyAdmin__factory, V3PositionValuator__factory, VaultController__factory
...
import { od, d, a, oa } from "../../../util/addresser"
...
const makeProposal = async (proposer: SignerWithAddress) => {
    ...
    //housekeeping - update implementation for Depricated CappedOAUSDC contract
    const upgradeData = await new ProxyAdmin__factory(proposer).
        attach(od.ProxyAdmin).populateTransaction.upgrade(
            od.CappedOAUSDC, od.TempImplementation
        )
    const upgradeForward = await new CrossChainAccount__factory().attach(od.optimismMessenger).
        populateTransaction.forward(od.ProxyAdmin, upgradeData.data!)
    console.log("Forward Data: ", upgradeForward)
    const upgrade = await ILayer1Messenger__factory.connect(a.OPcrossChainMessenger, proposer).
        populateTransaction.sendMessage(od.optimismMessenger, upgradeForward.data!, gasLimit)
    ...
    const proposalText = fs.readFileSync(path.resolve(__dirname, "./txt2.md"), 'utf8')
