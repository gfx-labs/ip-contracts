...
const deployProtocol = async (deployer: SignerWithAddress) => {
  ...
  const VaultControllerFactory = await ethers.getContractFactory(
    "VaultController"
  );
  ...
  const VaultController = await transparentFactory.deploy(
    uVC.address,
    proxyAdmin.address,
    "0x"
  );
  ...
  //USDI proxy
  const USDI = await transparentFactory.deploy(
    uUSDI.address,
    proxyAdmin.address,
    "0x"
  );
  ...
  const ThreeLinesFactory = new ThreeLines0_100__factory(deployer);
  ...
  //CURVE
  const curveFactory = new CurveMaster__factory().connect(deployer);
  const curve = await curveFactory.deploy();
  await curve.deployed();
  console.log("curve deployed: ", curve.address);
  {
    let t = await curve.setCurve(
  ...
  const UniswapRelayFactory = await ethers.getContractFactory(
  ...
  const chainlinkCompFactory = await ethers.getContractFactory(
  ...
  const chainlinkEthFactory = await ethers.getContractFactory(
    "ChainlinkOracleRelay"
  );
  ...
  const AnchorviewCompFactory = await ethers.getContractFactory(
    "AnchoredViewRelay"
  );
  ...
  const AnchorviewETHFactory = await ethers.getContractFactory(
    "AnchoredViewRelay"
  );
  ...
  await oracleMaster.setRelay(compAddress, AnchorviewComp.address);
  console.log("set comp relay");

  await oracleMaster.setRelay(wethAddress, AnchorviewETH.address);
