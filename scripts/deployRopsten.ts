...
const deployProtocol = async (deployer: SignerWithAddress) => {
  ...
  const VaultControllerFactory = new VaultController__factory().connect(
    deployer
  );
  ...
  //ORACLE THINGS
  //deploy oracleMaster
  const oracleMasterFactory = new OracleMaster__factory().connect(deployer);
  ...
  const regOracle = await VCcontract.registerOracleMaster(oracleMaster.address);
  await regOracle.wait();
  console.log("Registered oracle master");

  console.log("Creating uniswap comp relay");
  const UniswapRelayFactory = new UniswapV3OracleRelay__factory().connect(
    deployer
  );
...

  {
    ...
    let r1 = await oracleMaster.setRelay(compAddress, AnchorviewComp.address);
    await r1.wait();
    console.log("set comp relay");

    let r2 = await oracleMaster.setRelay(wethAddress, AnchorviewETH.address);
